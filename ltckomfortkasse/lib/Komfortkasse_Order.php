...
class Komfortkasse_Order
{
    ...
    public static function getOpenIDs()
    {
        ...
        $sql = 'SELECT id_order
				...
				WHERE 0 ';
        if ($use_prepayment)
            $sql .= ' or (o.current_state in (' . (int)$status_prepayment . ') and o.module in (' . (string)pSQL((string)self::quote((string)$methods_prepayment)) . '))';
        if ($use_invoice)
            $sql .= ' or (o.current_state in (' . (int)$status_invoice . ') and o.module in (' . (string)pSQL((string)self::quote((string)$methods_invoice)) . '))';
        if ($use_cod)
            $sql .= ' or (o.current_state in (' . (int)$status_cod . ') and o.module in (' . (string)pSQL((string)self::quote((string)$methods_cod)) . '))';
    ...
    public static function getInvoicePdf($invoiceNumber)
    {
        ...
        $sql = 'SELECT id_order_invoice FROM ' . (string)_DB_PREFIX_ . 'order_invoice o WHERE number=' . (string)pSQL((string)$invoiceNumber) . ' order by date_add desc limit 0, 1';
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        if (empty($result))
            return;
